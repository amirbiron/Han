...
def slug(heading):
    ...
    מחלקת התווים היא `[^\W_]` ולא `[A-Za-z0-9]`, מפני שהכלל של CommonMark חל על
    כל אות ולא רק על אותיות לטיניות. עם המחלקה הצרה `שם_ספר` היה מאבד את הקו
    התחתון שלו, בעוד שגיטהאב שומר עליו, וכל קישור לכותרת כזו היה מדווח כשבור.
    """
