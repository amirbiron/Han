...
ערכים קבילים: `small`, `medium`, `large` ו-`dynamic`. רצועה כופה את הגודל הזה; `dynamic` כופה סיווג אוטומטי לאותה ריצה, וכך אתה מקבל ריצה אחת מסווגת-אותות בפרויקט שהקונפיגורציה שלו קובעת רצועת ברירת מחדל. כל דבר אחר נחשב לחלק מההקשר הנגרר ולא לגודל, ולכן הוא לא מספק ערך מפורש והפענוח ממשיך במורד השרשרת: ה-`default-swarm-size` של `.han/config.md` של הפרויקט, אחריו זה של הקובץ האישי שלך, ורק כששניהם לא מספקים ערך שמיש, סיווג אוטומטי.
