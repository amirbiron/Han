...
| `han-feedback`       | הצטרפות מרצון | סקיל לאיסוף משוב אחרי סשן על ריצות של סקילים ב-Han                                                                                                       |
| `han-atlassian`      | הצטרפות מרצון | סקילים לפרסום מסמכים ופריטי עבודה למוצרי Atlassian                                                                                                       |
| `han-linear`         | הצטרפות מרצון | סקיל לפרסום פריטי עבודה ל-Linear (דורש שרת MCP של Linear)                                                                                                |
| `han-plugin-builder` | הצטרפות מרצון | נושא את ההנחיות והסקילים לבניית סקילים, סוכנים ופלאגינים משלך                                                                                            |
...
- [מפת התרגום](./docs/translation-status.md). איזה מסמך כבר קיים בעברית ואיזה עוד לא, לפי תיקייה. הקובץ נוצר על ידי `scripts/translation-status.py` ולא נכתב ביד, אז הוא תמיד משקף את מה שבדיסק.
- [גלוסרי התרגום](./docs/translation-glossary.md). המונחים הקבועים, מה מתורגם ומה נשאר באנגלית בכוונה, והמלכודות של מילים שנשמעות נכון ומשמעותן אחרת.
